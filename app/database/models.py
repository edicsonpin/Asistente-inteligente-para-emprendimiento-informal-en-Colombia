...
class Pais(Base):
    __tablename__ = "paises"  # Corregido: "paise" -> "paises"
...
class Departamento(Base):
    __tablename__ = "departamentos"  # Corregido: "departamento" -> "departamentos"
    ...
    negocios = relationship('Negocio', back_populates='departamento')
...
class Emprendedor(Base):
    ...
    departamento_residencia_id= Column(Integer, ForeignKey("departamentos.departamento_id"))
...
class Negocio(Base):
    ...
    #   departamento = relationship('Departamento', back_populates='negocios')
...
class ModeloIA(Base):
    ...
    # Nuevos campos para arquitectura híbrida
    arquitectura = Column(String(100))
    ...
    # Métricas específicas del documento
    mejora_precision = Column(Float)
    comparacion_linea_base = Column(JSON)
    ...
    # Campos para MLOps
    mlflow_registered = Column(Boolean, default=False)
    ...
    versiones_mlflow = relationship("VersionModeloMLflow", back_populates="modelo")
...
class EvaluacionRiesgo(Base):
    ...
    explicacion_contrafactual = Column(JSON)
    caracteristicas_sugeridas = Column(JSON)
    impacto_caracteristicas = Column(JSON)
    metricas_equidad = Column(JSON)
    variables_protegidas_analizadas = Column(JSON)
    ...
    embeddings = relationship("EmbeddingsCaracteristicas", back_populates="evaluacion")
