...
class GeneradorDatosPrueba:
    ...
    def listar_todos_los_paises(self):
        """Muestra una lista de todos los países encontrados en la BD."""
        print("\n" + "="*70)
        print("LISTA DE TODOS LOS PAÍSES EN LA BASE DE DATOS")
        print("="*70)
        
        try:
            # Consulta para obtener todos los objetos Pais
            paises = self.bd.query(Pais).order_by(Pais.nombre).all()
            
            if not paises:
                print("✗ ¡La tabla de países está vacía!")
                return []
            
            print(f"✅ Total de países encontrados: {len(paises)}")
            print("-" * 30)
            
            # Imprimir los detalles de cada país
            for pais in paises:
                print(f"  - {pais.emoji} {pais.nombre} (ID: {pais.pais_id}, ISO: {pais.codigo_iso})")
            
            print("-" * 30)
            return paises

        except Exception as error:
            print(f"✗ Error al consultar países: {str(error)}")
            return []
    ...
    def generar_emprendedores(self, usuarios):
        ...
        for idx, usuario in enumerate(usuarios):
            ...
            emprendedor = Emprendedor(
                ...
                departamento_residencia_id=1234,
